...
RSpec.describe Primer::State do
  it "renders content passed to it as a block" do
    result = render_string("<%= render Primer::State, color: :green, title: 'Hi' do %>content<% end %>")
  ...
  it "raises an error when color is not one of valid values" do
    exception = assert_raises ActionView::Template::Error do
      render_string("<%= render Primer::State, color: :chartreuse, title: 'Hi' do %>foo<% end %>")
    end
  ...
  it "assigns the correct CSS class for color" do
    result = render_string("<%= render Primer::State, color: :purple, title: 'Hi' do %>content<% end %>")
